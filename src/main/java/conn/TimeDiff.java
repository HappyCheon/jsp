...
public class TimeDiff {
	...
	// 외부로부터 날짜를 입력받아서 연산처리한후 다시 정수값으로 돌려준다.
	public int timeDiff(String wCdate) {
		...
		// 2-1. 날짜 연산을 위해서 문자형식의 날짜를 날짜형식으로 변경시켜준다.
		Date today = null;
		try {
		...
		// 1-2. DB에 저장된 날짜(2022-05-11 13:05:25.7)를 앞에 작업한 오늘날짜와 같은 포멧형식(yyyyMMddHHmm)으로 처리한다.
		...
		// 3. 날짜형식으로 변경된 두 자료(today/dateWdate)를 숫자형식(getTime())으로 변경처리한다.(시간으로 연산처리하기 위함) : 결과값 ms(1/1000)형식으로 반환한다.
